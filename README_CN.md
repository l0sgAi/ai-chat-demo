...
---
### MCP说明
> 不同的模型对于工具调用的能力有差异，目前测试下来工具调用效果最好的模型为`qwen-plus-latest`和`qwen-turbo-latest`，有的模型不支持`SpringAI`方式的工具调用，会报错，大家可以自己多尝试。
> 
> `MCP`需要开启对应的`MCP Server`，我已经上传至分支[mcp-server](https://github.com/l0sgAi/ai-chat-demo/tree/mcp-server)中。

##### 使用说明

1. 拉取分支`mcp-server`,配置自己的搜索`API KEY`，获取：[tavily-home](ttps://app.tavily.com/home)。
2. 启动`mcp-server`分支上的本项目。
3. 启动`main`分支上的本项目。
...
- [x] **(2025-07-27)** 全文检索会话。
- [x] **(2025-11-02)** 知识库管理，只有管理员才能进入。
- [x] **(2025-11-02)** RAG问答支持。
- [x] **(2025-11-11)** MCP联网搜索支持。
